...
//all movies wirh rating > 8

db.movies.find({
    rating:{$gt:8}
}).pretty()

// $gt, $gte -> greater, 4lt ->lesster,
//  $lte  -> lesser eqau,
//   $ne $in $nin-> not in


//filter keys -> projection
...
}, {summary:0, trailer:0}.pretty())

db.movies.find({_id:0, rating:{$gt:8}}, {name:1, rating:1}).pretty()
...
db.movies.find({}, {_id:0, name:1, rating:1}).sort({rating: -1}).limit(2).skip(2).pretty()
